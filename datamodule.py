    ...
    def train_dataloader(self):
        self.train_transforms = Compose(
            [
                ...
                # CropForegroundd(keys=["image3d"], source_key="image3d", select_fn=lambda x: x>0, margin=0),
                # CropForegroundd(keys=["image2d"], source_key="image2d", select_fn=lambda x: x>0, margin=0),
                RandZoomd(keys=["image3d"], prob=1.0, min_zoom=0.75, max_zoom=1.05, padding_mode='constant', mode=["trilinear"], align_corners=True), 
                RandZoomd(keys=["image2d"], prob=1.0, min_zoom=0.75, max_zoom=1.05, padding_mode='constant', mode=["area"]), 
